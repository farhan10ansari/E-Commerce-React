...
function updateButtonClicked(event)
{
    ...
    let seller_username=event.target.parentNode.parentNode.children[4].children[1].value;
...
function reload()
{
    getPaginationInfo();
    let req=new XMLHttpRequest();
    req.open("post","/getAdminProducts");
...
function load(products)
{
    ...
    for(let i=0;i<products.length;i++)
    {
        ...
        sample.querySelector(".seller_username").value=products[i].seller_username;
...
function getPaginationInfo()
{
    ...
    req.send(JSON.stringify({pagetype:"adminpage"}));
}
